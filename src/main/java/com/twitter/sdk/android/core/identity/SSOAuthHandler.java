...
class SSOAuthHandler extends AuthHandler {
    ...
    public static String availableSSOPackage(PackageManager packageManager) {
        ...
        return checkAppSignature(packageManager, DOGFOOD_PACKAGE_NAME, DOGFOOD_SIGNATURE) ? DOGFOOD_PACKAGE_NAME : null;
    }
    ...
    private static boolean checkAppSignature(PackageManager packageManager, String str, String str2) {
        try {
            for (Signature toCharsString : packageManager.getPackageInfo(str, 64).signatures) {
                if (!str2.equals(toCharsString.toCharsString())) {
                    return false;
                }
            }
            return true;
