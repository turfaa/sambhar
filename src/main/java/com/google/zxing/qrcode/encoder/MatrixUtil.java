...
final class MatrixUtil {
    ...
    static void embedTypeInfo(ErrorCorrectionLevel errorCorrectionLevel, int i, ByteMatrix byteMatrix) throws WriterException {
        ...
        for (i = 0; i < bitArray.getSize(); i++) {
            ...
            byteMatrix.set(TYPE_INFO_COORDINATES[i][0], TYPE_INFO_COORDINATES[i][1], z);
            if (i < 8) {
    ...
    private static void maybeEmbedPositionAdjustmentPatterns(Version version, ByteMatrix byteMatrix) {
        if (version.getVersionNumber() >= 2) {
            ...
            versionNumber = POSITION_ADJUSTMENT_PATTERN_COORDINATE_TABLE[versionNumber].length;
            for (int i = 0; i < versionNumber; i++) {
                for (int i2 = 0; i2 < versionNumber; i2++) {
                    int i3 = iArr[i];
                    int i4 = iArr[i2];
